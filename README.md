...
一個純前端的小工具，用來把全聯店點 CSV 轉成可開啟的 Google Maps 分段路線，方便跑店前先整理路線並儲存連結。

此工具會呼叫 Google Maps / Places / Routes API 協助定位與排序，但無法保證是全域最佳、最快路線，若店點很多或實際騎乘路況複雜，建議將輸出結果當初稿，再由使用者人工確認與進行調整。
...
6. 將結果拆成多段 Google Maps URL 和產生可下載的 `routes.txt`。
...
> 注意：該 repo 是純前端工具，API key 會在瀏覽器端使用，請勿使用未受限制的正式 API key。
...
5. 上傳 CSV。
6. 點選「產生路線」。
7. 在輸出頁可開啟各段 Google Maps 路線和下載 `routes.txt`。

## 重要限制

### 店點數量限制

Routes API 對中繼點數有上限，因此該版本目前僅適用於少量店點，不適合直接處理大量店點並期待全域最佳結果。
...
> 偏向機車使用情境的路線初稿，並非最快路線的機車導航。

### 分段路線限制

Google Maps URL / 手機 app 對單一路線可接受的停靠點數有限，工具會依「每段停靠點」的設定將最終路線拆成多段，建議維持保守數量，例如 6–8 個停靠點。
...
- 請勿將 API key 送交到 repo。
- 請勿在公開 repo 中寫死 API key。
- 使用前請限制 API key 的來源網域與可用 API。
